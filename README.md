...
This project includes simple role-based authorization with three roles: `user`, `worker`, and `admin`.

- Users have a `role` column on the `users` table and are cast to an enum `App\Enums\Role`.
- Middleware alias `role` is available to protect routes.
...
// Admins OR workers
Route::middleware(['auth', 'role:admin,worker'])->get('/staff', fn () => 'staff area');
```

Seeding creates three convenience accounts:

- admin@example.com / password (role: admin)
- worker@example.com / password (role: worker)
